...
local lossTrainVector = torch.Tensor(opt.epoch)
local lossTestVector = torch.Tensor(opt.epoch)
...
repeat
  ...
  log:plot()
  lossTrainVector[epoch] = LossTrain
  lossTestVector[epoch] = LossTest
  epoch = epoch + 1
...
gnuplot.pngfigure('loss_1.png')
gnuplot.plot({'trainLoss',lossTrainVector},{'testLoss',lossTestVector})
